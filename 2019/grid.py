...
class Grid:
    ...
    def draw_grid(self, color_map=DEFAULT_COLOR_MAP, cell_size=10, extra_text=None, extra_text_rows=None, font_size=14, image_copies=1, extra=None, extra_callback=None):
        ...
        if extra_text is not None or extra_text_rows > 0:
            if self.fonts is None:
                self.fonts = [
                    ImageFont.truetype(os.path.join('Helpers', 'Font-SourceCodePro-Bold.ttf'), int(float(font_size) * 1.5)),
                    ImageFont.truetype(os.path.join('Helpers', 'Font-DSEG14Classic-Bold.ttf'), font_size),
                ]
            fnt_source, fnt_segment = self.fonts
